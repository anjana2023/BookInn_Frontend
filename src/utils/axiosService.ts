...
import logout, { ownerlogout } from "./logout";
...
const getAccessToken = async () => {
  try {
    ...
    if (role === "owner" || role === "user") {
      ...
      else if (user?.owner?.isBlocked)
        ownerlogout("Your account has been blocked by administrator", "error");
    }  
    return token;
  } catch (error) {
